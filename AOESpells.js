    ...
    <form>
      <div class="form-group">
        ...
        <div class="form-fields buttons radios">
          <input type="radio" name="shape" id="emanation" value="emanation">
          <label for="emanation" onclick="toggleEmanationMessage(true)"><i class="fas fa-square"></i> Emanation</label>
          <input type="radio" name="shape" id="burst" value="burst" checked>
      ...
      <div class="notes" id="emanation-message">
        ${token ? `
          ...
          ${game["modules"].has("token-attacher") ? `➡️ The template will move with the selected token.` : `⚠️ For the template to move with the selected token, please install and activate the module "Token Attacher".`}
        ` : `
      ...
      <div class="form-fields">
        <input type="range" id="radius" name="radius" value="2.5" min="1" max="120" step="5" oninput="updateRangeValue(this.value);">
        <span id="range-value" class="range-value">2.5</span>
      </div>
      </div>
    </form>
    <script>
...
//   angle: 57,
   ...
   texture: "modules/jb2a_patreon/Library/2nd_Level/Flaming_Sphere/Opacities/FlamingSphere_01_Orange_75OPA_200x200.webm"
  };
...
for await (const token of canvas.tokens.controlled) {
  ...
  let existing = token.actor.data.items.find(i => i.type === 'spell' && i.name === 'Flaming Sphere');
  if (existing) {
          let spellID = existing._id;
          console.log(spellID);
          game.pf2e.rollItemMacro(spellID);
  } else {
      ui.notifications.warn("Bitch you ain't got a spell named Fireball");  
      return;
  }
}
})(); 